---
title: "Build Cross-Language Multi-Agent Team with Google's Agent Development Kit and A2A"
url: "https://developers.googleblog.com/build-cross-language-multi-agent-team-with-google-agent-development-kit-and-a2a/"
date: ""
author: ""
feed_url: "https://developers.googleblog.com/feed/"
---
This post demonstrates how Python and Go agents can collaborate on contract compliance using the Agent2Agent protocol for seamless cross-language coordination.
